...
import { VisualAidRequest, VisualAidResponse } from '../../types/agentic';
import AgenticService from '../../services/agenticService';
import LoadingSpinner from '../../components/ui/LoadingSpinner';

export default function VisualAidsGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await AgenticService.generateVisualAids(formData);
      setResult(response);
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Generate Visual Aid</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Enter the topic for visual aid"
              required
            />
          </div>
      ...
      {result && (
        ...
        >
          <div className="mb-4">
            <h3 className="text-xl font-semibold text-gray-800">Generated Visual Aid</h3>
          ...
          <div className="aspect-w-16 aspect-h-9 bg-gray-100 rounded-lg overflow-hidden mb-4">
            <img
              src={result.visual_aid.url}
              alt={result.visual_aid.alt_text}
              className="object-contain w-full h-full"
            />
          </div>

          <div className="text-center text-gray-600 mt-2">
            {result.visual_aid.caption}
          </div>

          <div className="mt-4 pt-4 border-t border-gray-200 text-sm text-gray-500">
            <div>Type: {result.visual_aid.type}</div>
